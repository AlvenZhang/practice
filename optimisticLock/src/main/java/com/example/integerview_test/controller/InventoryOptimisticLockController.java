...
public class InventoryOptimisticLockController {
    ...
    public String placeOrder(@PathVariable("courseId") Integer courseId) {
        ...
        InventoryOptimisticLock one = inventoryOptimisticLockService.getOne(queryWrapper);
        ...
        if (one.getNums() > 0) {
            ...
            while (!update) {
                log.warn("版本不正确，开始重试。。。。");
                one = inventoryOptimisticLockService.getOne(queryWrapper);
